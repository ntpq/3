...
const ERC20 = observer(() => {
  ...
  const store = useLocalStore(() => ({
    ...
    async onSubmit() {
      ...
      const res = await store.curToken.transfer.call({
        params: [store.receiverAdderss.value, store.amount.value.toFixed(0, 1)],
      })
    ...
    async onApprove() {
      ...
      const res = await store.curToken.approve.call({
        params: [store.receiverAdderss.value, store.amount.value.toFixed(0, 1)],
      })
      const receipt = await res.wait()
      if (receipt.status) {
    ...
    <Container maxW="md">
      <form>
        <FormControl mt={20}>
          <TokenInput
            amount={store.amount}
            token={store.curToken}
            onChangeAmount={(e) => store.amount.setFormat(e)}
