...
export class TokenStore {
  ...
  constructor(rootStore: RootStore) {
    ...
    this.tokens = {
      [BSCMainnetConfig.chainId]: pancakeTokenList.tokens
        .concat(this.localStorageToken.value[BSCMainnetConfig.chainId] || [])
  ...
  async loadPrivateData() {
    ...
    await this.currentNetwork.multicall([
      ...this.currentTokens
        .filter((i) => !i.isEther)
        .map((i) =>
          helper.c.preMulticall(i, { method: 'balanceOf', params: [this.currentNetwork.account], handler: i._balance })
        ),
    ])
